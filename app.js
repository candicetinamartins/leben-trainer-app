...
const UI_TRANSLATIONS = {
    ...
    // ... (Keep other languages as provided in original file) ...
...
class FlashcardApp {
    ...
    // --- NEW: Render Helper for Language Pill ---
    ...
    renderQuestionCard() {
        ...
        // Images logic (simplified for mobile)
        let imageSection = '';
    ...
    renderFeedbackDrawer(selectedAnswer, correctAnswer, selectedIndex) {
        ...
        buttons.forEach((btn, idx) => {
            ...
            // Reveal translation after answering
        ...
        // Load the drawer ad (only if consent granted)
        if (hasAdConsent()) {
            try { (adsbygoogle = window.adsbygoogle || []).push({}); } catch(e) {}
        }

        // Add extra bottom space so user can scroll past the drawer
        const scrollArea = document.getElementById('question-scroll-area');
        if (scrollArea) {
            scrollArea.style.paddingBottom = '20rem';
        }

        // Auto-scroll selected answer into view above the drawer
        setTimeout(() => {
